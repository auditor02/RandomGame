...
contract Attack {
    ...
    // Attacks the `Game` contract by guessing the exact number because `blockchain` and `block.timestamp`
    // are accessible publicly
    function attack() public {
        // `abi.encodePacked` takes in the two params - `blockchain` and `block.timestamp`
        ...
        // keccak256 is a hashing function which takes in a bytes array and converts it into a bytes 32
